...
const marketplaces = [
  { name: "Wildberries", color: "bg-purple-100 text-purple-700" },
  { name: "Ozon", color: "bg-blue-100 text-blue-700" },
  { name: "Apteka.ru", color: "bg-green-100 text-green-700" },
  { name: "Сбер Аптека", color: "bg-emerald-100 text-emerald-700" },
];
  ...
  return (
    <section id="buy" className="py-16 md:py-24 bg-muted/50">
      <div className="max-w-4xl mx-auto px-4">
        <h2 className="text-3xl md:text-4xl font-bold text-center mb-3">Где купить</h2>
        <p className="text-center text-muted-foreground mb-12">Скоро на ведущих площадках</p>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          {marketplaces.map((m) => (
            <div key={m.name} className="bg-card rounded-2xl p-5 border border-border text-center">
        ...
        <div className="bg-card rounded-2xl p-8 border border-border text-center max-w-lg mx-auto">
          ...
          <p className="text-muted-foreground mb-4">
            Оставьте email — сообщим когда появится в продаже
          </p>
          {submitted ? (
            <div className="flex items-center justify-center gap-2 text-accent font-medium">
              <CheckCircle2 className="w-5 h-5" />
              <span>Спасибо! Мы вас уведомим.</span>
            </div>
          ) : (
